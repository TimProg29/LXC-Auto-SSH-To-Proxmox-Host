...
echo "→ Prüfe ob Container $LXC_ID erreichbar ist"
pct status "$LXC_ID" >/dev/null

echo "→ Stelle sicher, dass Node vorbereitet ist (bootstrap)"
...
echo "→ Erzeuge Key im LXC $LXC_ID: $KEY_PATH"
...
# Duplikate vermeiden: wenn derselbe Public Key schon existiert, nichts tun
if grep -Fq "$PUBKEY" "$AUTHORIZED_KEYS"; then
  echo "ℹ Key existiert bereits in authorized_keys (kein doppelter Eintrag)"
else
